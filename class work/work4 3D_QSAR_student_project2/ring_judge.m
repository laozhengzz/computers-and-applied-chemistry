...
for i = 1:size(input_structure,1)
    ...
    for k = 2:size(candidate_atom,1)
        if norm(candidate_atom(k,1:3) - candidate_atom(1,1:3))<=R1
            p_1 = candidate_atom(k,:);
            %trig = 1;
            for l = 2:size(candidate_atom,1)
                if norm(candidate_atom(l,1:3) - p_1(1,1:3))<=R1 && k ~=l
                    p_2 = candidate_atom(l,:);
                    if norm(candidate_atom(l,1:3) - candidate_atom(1,1:3))<=R1
                    ...
                    for m = 2:size(candidate_atom,1)
                        if norm(candidate_atom(m,1:3) - p_2(1,1:3))<=R1 && l ~=m && k ~=m
                            p_3 = candidate_atom(m,:);
                            if norm(candidate_atom(m,1:3) - candidate_atom(1,1:3))<=R1
                                ring_atom(ran,1) = input_structure(i,7);
                                ring_atom(ran,2) = candidate_atom(k,7);
                                ring_atom(ran,3) = candidate_atom(l,7);
                            ...
                            for n = 2:size(candidate_atom,1)
                                if norm(candidate_atom(n,1:3) - p_3(1,1:3))<=R1 && m ~=n && l ~=n && k ~=n
                                    p_4 = candidate_atom(n,:);
                                    if norm(candidate_atom(n,1:3) - candidate_atom(1,1:3))<=R1
                                        ring_atom(ran,1) = input_structure(i,7);
                                        ring_atom(ran,2) = candidate_atom(k,7);
                                        ring_atom(ran,3) = candidate_atom(l,7);
                                        ring_atom(ran,4) = candidate_atom(m,7);
                                    ...
                                    for p = 2:size(candidate_atom,1)
                                        if norm(candidate_atom(p,1:3) - p_4(1,1:3))<=R1 && n ~=p && m ~=p && l ~=p && k ~=p
                                            p_4 = candidate_atom(p,:);
                                            if norm(candidate_atom(p,1:3) - candidate_atom(1,1:3))<=R1
                                                ring_atom(ran,1) = input_structure(i,7);
                                                ring_atom(ran,2) = candidate_atom(k,7);
                                                ring_atom(ran,3) = candidate_atom(l,7);
                                                ring_atom(ran,4) = candidate_atom(m,7);
